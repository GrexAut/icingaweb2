...
class Dashboard extends BaseHtmlElement
{
    ...
    protected function assemble()
    {
        ...
        if ($activeHome && ! empty($activeHome->getPanes(true))) {
            ...
            $this->setAttribute('class', 'content');
            $format = Html::Sprintf(
                t('Currently there is no pane available. This might change once you enabled some of the available %s.'),
                new Link('modules', 'config/modules')
            );

            if (Url::fromRequest()->getPath() === self::BASE_ROUTE) {
                $format = Html::sprintf(
                    t('Currently there is no dashboards available. You start with creating new pane from %s'),
                    new Link('Available Dashlets', self::BASE_ROUTE . '/home?home=' . DashboardHome::AVAILABLE_DASHLETS)
                );
            }
